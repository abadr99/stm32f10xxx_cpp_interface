...
Spi::Spi(const SpiConfig& config) :config_(config) {
    switch (config_.number) {
        case kSPI1 : spi_reg = (reinterpret_cast<volatile SpiRegDef*>(Addr<Peripheral::kSPI1 >::getBaseAddr())); break;
        case kSPI2 : spi_reg = (reinterpret_cast<volatile SpiRegDef*>(Addr<Peripheral::kSPI2 >::getBaseAddr())); break;
